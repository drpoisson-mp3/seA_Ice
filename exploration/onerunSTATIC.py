...
u0=np.abs(trainpd['u_buoy'])<=1e-5
v0=np.abs(trainpd['v_buoy'])<=1e-5
nospeed=u0*v0
trainpd['static']=nospeed
...
target = pd.DataFrame(trainpd['static'])# 

# weight for the static case
total_cases=len(trainpd)
total_static= (trainpd['static']).sum()
...
trainpd = trainpd.drop(['u_buoy', 'v_buoy','buoynorm','static'],axis=1) # Take away the time of the year, wrong dtype and seems to be useless anyway
...
#forcing some tuning:
pos_weight*=4#torch.tensor([64]).to(device)
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
rlosses={}
losses=[]
print(pos_weight.item(), 'times rewarded for Guessing static correct')
...
for epoch in range(epoch):
    ...
    curpercent=0
    rl=[]
    optimizer = optim.Adam(babygpu.parameters(),lr=lr[epoch]) #
...
torch.save(babygpu.state_dict(), f'.//weights//babygpu_{labels[0].item()}_log1pNorms_{traintensor.shape[1]}input_{epoch}E_weight{round(pos_weight.item())}.pt')
...
plt.savefig(f'.//outputs//babygpu_{labels[0].item()}_log1pNorms_{traintensor.shape[1]}inputs_{epoch}E_weight{round(pos_weight.item())}.png')
